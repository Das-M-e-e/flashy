...
function applyRemote(snapshot: Snapshot, sha: string): void {
...
async function runSync(config: db.SyncConfigRow): Promise<void> {
  ...
  if (!dirty) {
    applyRemote(parseSnapshot(remote.content), remote.sha);
    return;
  }
...
export async function resolve(choice: "local" | "remote"): Promise<SyncStatus> {
  ...
  try {
    if (choice === "remote") {
      applyRemote(conflict.remoteSnapshot, conflict.remoteSha);
    } else {
